...
def objfun(parameters, syst, targets, imgcon):
    ...
    no_points = imgcon.objpoints.shape[0]
...
if __name__ == "__main__":
    ...
    translations = np.empty((5, 3))
    costs = []

    imgpath = "testimgs5/good"
    imgext = "*.TIF"
    ...
    for num_imgs in range(5, 6):
        no_imgs_considered = num_imgs
        ...
        RTbounds = np.vstack(([(-np.inf, np.inf)] * 2,
                             (400, 800),  # Translations to target in z direction.
